...
(function () {
  ...
  var pictures = scene.querySelectorAll('.nt-scene__pic'), messages = scene.querySelectorAll('.nt-scene__msg');
  ...
  function restore() {
    ...
    [scene,stage].concat(Array.from(frames),Array.from(pictures),Array.from(messages),Array.from(cards),Array.from(images)).forEach(function(e){e.removeAttribute('style');});
  }
  function layout() {
    if (!enabled) return;
    height = Math.round(Math.max(260,Math.min(innerHeight-32,scene.clientWidth*.75+140)));
    offset = Math.round(Math.max(0,(innerHeight-height)/2));
  ...
  function tick() {
    ...
    var target=clamp((offset-top)/Math.max(1,scene.offsetHeight-height));
